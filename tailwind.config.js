...
/**
 * Crepitus protocol Tailwind configuration.
 *
 * Extends the default scaffold with the protocol's design system: the
 * chartreuse collateral ramp, the neutral surface scale, and the
 * liquid-glass component utilities used throughout the dashboard.
 * The plugin block below registers custom component classes so they
 * are tracked by the Tailwind engine rather than living only in CSS.
 */

const crepitusConfig: Config = {
  ...
  plugins: [
    function ({ addComponents }: { addComponents: (classes: Record<string, Record<string, unknown>>) => void }) {
      addComponents({
        '.liquid-glass': {
        ...
        '.dot-matrix-bg': {
          'background-image': 'radial-gradient(circle, rgba(200, 230, 0, 0.03) 1px, transparent 1px)',
